...
public class OverflowLimbo {
    ...
    public static void main(String[] args) {
        ...
        DynamicRegistry<DimensionType> dimensionManager = MinecraftServer.getDimensionTypeRegistry();
        DimensionType THE_VOID = DimensionType.builder()
                .ultrawarm(false)
                .natural(false)
                .piglinSafe(false)
                ...
                .logicalHeight(384)
                .build();
        dimensionManager.register("overflow:the_void", THE_VOID);

        // World Container
        logger.info("Initializing world...");
        InstanceContainer world = MinecraftServer.getInstanceManager().createInstanceContainer(DynamicRegistry.Key.of("overflow:the_void"));
        ...
        events.addListener(AsyncPlayerPreLoginEvent.class, event -> {
            ...
            if (world.getPlayers().size() >= config.getPlayerLimit() && config.getPlayerLimit() > -1) {
                ...
                logger.info("{} [{}] connected.", player.getUsername(), player.getPlayerConnection().getRemoteAddress());
            }
        });
